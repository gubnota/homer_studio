...
export function ReviewPage({ project, onProjectChange }: { project: ProjectSnapshot | null; onProjectChange: (project: ProjectSnapshot) => void }): JSX.Element {
  ...
  async function convertRecording(chapterId: string, segmentId: string, bytes: number[]): Promise<void> {
    setBusyId(segmentId); setError('')
    try {
      await waitForJob(await productionApi.convertSegmentRecording(activeProject, chapterId, segmentId, bytes))
...
function SectionRecorder({ disabled, onConvert }: { disabled: boolean; onConvert: (bytes: number[]) => void }): JSX.Element {
  ...
  return <div className="section-recorder"><p>Read this section with your preferred timing and emphasis. Original Chatterbox will convert your delivery to the chosen narrator voice. Listen to the new take before choosing it.</p><div className="actions"><button disabled={disabled} onClick={() => recording ? stop() : void start()}>{recording ? 'Stop recording' : 'Start recording'}</button>{clip && <button className="primary" disabled={disabled} onClick={() => void convert()}>Convert to narrator voice</button>}</div>{recording && <span>Recording · stops after 20 seconds</span>}{previewUrl && <audio controls src={previewUrl} />}{error && <div className="inline-error">{error}</div>}</div>
}

function ChapterAudio({ project, chapter, onEditCue }: { project: ProjectSnapshot; chapter: Chapter; onEditCue: (order: number) => void }): JSX.Element {
  const [url, setUrl] = useState('')
  const [waveform, setWaveform] = useState<number[]>([])
  const [activeCue, setActiveCue] = useState(-1)
  const [selectedCue, setSelectedCue] = useState(-1)
  const canEditCues = chapter.cues.length === chapter.segments.length && chapter.cues.length > 0
  const selectedRange = chapter.cues[selectedCue]
  ...
  return url ? <div className="chapter-player">{waveform.length > 0 && <div className="waveform" role={canEditCues ? "button" : undefined} tabIndex={canEditCues ? 0 : undefined} aria-label={canEditCues ? "Select a spoken section on the waveform" : "Audio waveform"} onClick={(event) => { if (!canEditCues) return; const ratio = (event.clientX - event.currentTarget.getBoundingClientRect().left) / event.currentTarget.clientWidth; const ms = ratio * (chapter.audioDurationMs ?? 0); const cue = chapter.cues.findIndex((item) => ms >= item.startMs && ms < item.endMs); setSelectedCue(cue); if (cue >= 0 && audio.current) audio.current.currentTime = (chapter.cues[cue]?.startMs ?? 0) / 1000 }} onKeyDown={(event) => { if (canEditCues && event.key === "Enter") { event.preventDefault(); setSelectedCue(activeCue >= 0 ? activeCue : 0) } }}>{waveform.map((peak, index) => <i key={index} className={selectedRange && chapter.audioDurationMs && index / waveform.length >= selectedRange.startMs / chapter.audioDurationMs && index / waveform.length <= selectedRange.endMs / chapter.audioDurationMs ? "selected" : ""} style={{ height: `${Math.max(8, peak * 100)}%` }} />)}</div>}{selectedRange && canEditCues && <div className="waveform-selection"><span>Selected: {formatDuration(selectedRange.startMs)}–{formatDuration(selectedRange.endMs)}</span><button onClick={() => onEditCue(selectedCue)}>Record replacement</button></div>}<audio ref={audio} controls preload="metadata" src={url} onTimeUpdate={(event) => { const ms = event.currentTarget.currentTime * 1000; setActiveCue(chapter.cues.findIndex((cue) => ms >= cue.startMs && ms < cue.endMs)) }} />{chapter.cues.length > 0 ? <div className="spoken-cues" aria-label="Spoken lines">{chapter.cues.map((cue) => <button key={cue.order} className={activeCue === cue.order ? 'active' : ''} onClick={() => { if (audio.current) audio.current.currentTime = cue.startMs / 1000 }}>{cue.text}</button>)}</div> : <span className="field-note">Line timing is available for newly generated narration.</span>}</div> : <span>Preparing player…</span>
}
