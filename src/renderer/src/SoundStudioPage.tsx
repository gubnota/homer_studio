...
export function isValidSoundRequest(request: SoundRequest): boolean {
  ...
  return validLegacyGesture && request.prompt.trim().length > 0 && request.prompt.length <= 500 && (request.negativePrompt?.length ?? 0) <= 300 && request.durationSeconds >= 1 && request.durationSeconds <= (request.category === 'sound_effect' ? 120 : 20) && (request.seed === null || (Number.isInteger(request.seed) && request.seed >= 0 && request.seed <= 2147483647))
}
