...
function engineFor(category: SoundCategory): string { return category === 'sound_effect' ? 'stable_audio_open' : 'chatterbox_turbo' }
...
export function SoundStudioPage(): JSX.Element {
  ...
  const [prompt, setPrompt] = useState('Soft cotton fabric rustling close to the microphone')
  ...
  const selectedWorker = workers.find((worker) => worker.engine === engineFor(category))
  ...
  return <div className="page sound-page">
    ...
    <div className="sound-grid">
      <section className="panel sound-compose">
        <h2>Create a clip</h2>
        <label>Kind of audio<select value={category} onChange={(event) => setCategory(event.target.value as SoundCategory)}>{categories.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label>
      ...
      <aside className="panel sound-workers"><h2>Local models</h2>{['chatterbox_turbo', 'stable_audio_open'].map((engine) => { const worker = workers.find((item) => item.engine === engine); return <div key={engine} className="sound-worker"><strong>{engine === 'chatterbox_turbo' ? 'Chatterbox Turbo' : 'Stable Audio Open'}</strong><span className={worker?.ready ? 'ready' : 'unavailable'}>{worker?.ready ? 'Ready' : 'Unavailable'}</span><small>{worker?.message ?? 'Checking worker…'}</small></div> })}<p className="field-note">Models run in separate local workers. Configure their addresses in Settings and see setup instructions in workers/README.md.</p></aside>
    </div>
